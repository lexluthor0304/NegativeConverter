...
- 🧪 **Expired film rescue**: a separate entry and flow for aged rolls, negatives and positives alike — fog, overall cast, shadow/highlight crossover and lost speed are measured on the positive and corrected per channel, and OpenCV.js measures what varies across the frame (uneven fog surface, local contrast), with a diagnosis and seven strengths on the preview, the 16-bit export and batch exports (see `docs/expired-film-rescue.md`)
