...
export function deepCopySanitizedSettings(safe, { autoFrameMeta = safe.autoFrameMeta } = {}) {
  return {
    ...
    semanticMap: safe.semanticMap ? structuredClone(safe.semanticMap) : null,
    learnedDefaults: safe.learnedDefaults ? { ...safe.learnedDefaults } : null,
    reviewed: Boolean(safe.reviewed),
    wbUserOverride: Boolean(safe.wbUserOverride),
    wbSemanticApplied: Boolean(safe.wbSemanticApplied),
    filmType: safe.filmType,
    positiveMode: safe.positiveMode,
    filmTypeSource: safe.filmTypeSource,
    filmTypeConfidence: safe.filmTypeConfidence,
    filmTypeReason: safe.filmTypeReason,
    ...
    coreCyan: safe.coreCyan ?? 0,
    corePaper: safe.corePaper || 'none',
    ...
    repairStrokes: structuredClone(safe.repairStrokes || []),
    flatFieldId: safe.flatFieldId || null,
    look: safe.look ? structuredClone(safe.look) : null,
    // Expired-film rescue: strengths plus the measured analysis of this frame.
    expiredEnabled: Boolean(safe.expiredEnabled),
    expiredLevels: safe.expiredLevels,
    expiredNeutralize: safe.expiredNeutralize,
    ...
    frameMetadata: safe.frameMetadata ? { ...safe.frameMetadata } : { frameNumber: '', notes: '' },
