...
export const i18n = {
      zh: {
        ...
        exportJpeg: "导出 JPEG",
        exportTiff: "导出 TIFF",
        dustAiTitle: "AI 修复 · MI-GAN（本机运行）",
        dustAiEnable: "提交和导出时用学习型补绘填充遮罩",
        dustAiLoad: "加载模型",
        dustAiPick: "使用模型文件（.onnx）",
        dustAiStatusIdleGpu: "尚未加载模型。勾选 AI 修复即可加载内置 MI-GAN；优先使用 GPU，失败时使用 CPU。",
        dustAiStatusIdleWasm: "尚未加载模型。这个浏览器没有 WebGPU，模型会在 WASM 上运行（较慢）。",
        dustAiStatusLoading: "正在加载模型… {percent}%",
        dustAiStatusReady: "模型就绪：{source}，运行于 {provider}",
        dustAiStatusLast: "上次 {tiles} 个分块，{ms} 毫秒",
        dustAiStatusError: "AI 不可用：{message}。已使用普通修复；请重试或加载 MI-GAN Pipeline ONNX 文件。",
      ...
      en: {
        ...
        dustAiTitle: "AI repair · MI-GAN (on-device)",
        dustAiEnable: "Fill the mask with the learned inpainter on commit and export",
        dustAiLoad: "Load model",
        dustAiPick: "Use a model file (.onnx)",
        dustAiStatusIdleGpu: "No model loaded. Enable AI repair to load bundled MI-GAN; tries GPU with CPU fallback.",
        dustAiStatusIdleWasm: "No model loaded. This browser has no WebGPU, so the model would run on WASM (slower).",
        ...
        dustAiStatusError: "Model failed: {message}. Using basic repair. Retry or load a MI-GAN Pipeline ONNX file.",
      ...
      ja: {
        ...
        exportJpeg: "JPEG出力",
        exportTiff: "TIFF出力",
        dustAiTitle: "AI 修復 · MI-GAN（端末内で実行）",
        dustAiEnable: "確定と書き出し時に学習済みインペインタでマスクを埋める",
        dustAiLoad: "モデルを読み込む",
        dustAiPick: "モデルファイルを使う（.onnx）",
        dustAiStatusIdleGpu: "モデル未読み込み。AI 修復をオンにするかモデルを読み込んでください。GPU を優先し、失敗時は CPU で実行します。",
        dustAiStatusIdleWasm: "モデル未読み込み。このブラウザには WebGPU がないため WASM で実行します（遅め）。",
        dustAiStatusLoading: "モデルを読み込み中… {percent}%",
        dustAiStatusReady: "モデル準備完了：{source}（{provider}）",
        dustAiStatusLast: "前回 {tiles} タイル、{ms} ms",
        dustAiStatusError: "AI を利用できません：{message}。通常の修復を使用します。再試行するか MI-GAN Pipeline の ONNX ファイルを選んでください。",
