...
export function detectDust(imageData, { strength = 3 } = {}) {
  ...
  for (let i = 0; i < pixelCount; i++) {
    ...
    // multiply: both are 0-255, result clamped
    increase[i] = Math.min(255, (val * fullGauss[i]) >> 8);
  }
  ...
  return {
    mask,
    particleCount,
    _state: { scharrAll: fullScharrAll, linien: fullLinien, gauss: fullGauss, width: w, height: h }
  };
}
...
export function updateDustStrength(imageData, existingState, newStrength) {
  ...
  for (let i = 0; i < pixelCount; i++) {
    ...
    increase[i] = Math.min(255, (val * gauss[i]) >> 8);
  }
  ...
  return {
    mask,
    particleCount,
    _state: { scharrAll, linien, gauss, width: w, height: h }
  };
}
